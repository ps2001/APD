...
int* get_dst(int rank, int numProcs, int leader) {
	...
	*/
	...
	for (int i = 0; i < num_neigh; i++) {
		if (neigh[i] != v[rank]) {
			MPI_Recv(vRecv, numProcs, MPI_INT, neigh[i], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	...
	*/
	...
	if (rank != leader) {
		...
		MPI_Recv(vRecv, numProcs, MPI_INT, v[rank], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for (int i = 0; i < num_neigh; i++) {
			...
			int tmp_leader;
			MPI_Recv(&tmp_leader, 1, MPI_INT, neigh[i], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			leader = max(leader, tmp_leader);
		}
	}
...
int ** get_topology(int rank, int nProcesses, int * parents, int leader) {
    ...
    if (parents[i] == rank) {
      for (int j = 0; j < nProcesses; j++) {
        MPI_Recv(vTopology[j], nProcesses, MPI_INT, i, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
